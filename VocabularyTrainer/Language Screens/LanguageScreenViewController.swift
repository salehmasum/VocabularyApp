...
class LanguageScreenViewController: UIViewController, UISearchBarDelegate, MFMailComposeViewControllerDelegate {
    ...
    fileprivate func applyAutolayoutNow() {
   ...
   @objc func sortButtonTapped(_ sender: UIButton) {
    ...
    """
        ...
        if let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            // writing
            do {
                try exportString.write(
                    to: fileURL, atomically: false, encoding: .macOSRoman
                )
                let alert = UIAlertController(
                    title: "\(NSLocalizedString("Export successful:", comment: "Export successful:")) \(language).csv",
