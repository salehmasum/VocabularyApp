...
//
    ...
    private func setUpInterfaceStyleUI() {
        ...
        case .light, .unspecified:
            addNewWordLabel.textColor = UIColor.label
            
            newWordTextField.backgroundColor = .secondarySystemBackground
            newWordTextField.textColor = .label
            
            translationTextField.backgroundColor = .secondarySystemBackground
        ...
        if isEnabled {
            UIView.animate(withDuration: 0.3) { [weak self] in
               // self?.addButton.backgroundColor = BackgroundColor.mediumSpringBud
                self?.addButton.backgroundColor = .secondaryLabel
            ...
            UIView.animate(withDuration: 0.3) { [weak self] in
               // self?.addButton.backgroundColor = UIColor(red: 207/255, green: 207/255, blue: 205/255, alpha: 0.4)
                self?.addButton.backgroundColor = .secondarySystemBackground
    ...
    func showAlert() {
        let alertTitle = String.localizedStringWithFormat(
            NSLocalizedString(
                "The word %@ already exists. Adding the new pair will overwrite the old pair of words.",
                                                                            
                comment: "The word %@ already exists. Adding the new pair will overwrite the old pair of words."
            ),
            newWordTextField.text ?? ""
        )
        
        let duplicateWordAlert = UIAlertController(
            title: "Duplicate Word Detected",
            message: alertTitle,
    ...
    @objc private func textFieldChanged(_ sender: UITextField) {
        ...
        var hasText = false
        if let senderText = sender.text?.trimmingCharacters(in: .whitespaces), !senderText.isEmpty {
            hasText = true
        }
